import { pairOpponentRequestSchema } from "@gamepool/shared";
...
export async function POST(request: Request, { params }: Params) {
  ...
  return withUser(request as never, async ({ userId }) => {
    const body = await parseBody(request as never, pairOpponentRequestSchema);
    return service.pair(userId, id, body);
  });
}
